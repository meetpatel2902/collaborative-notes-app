...
const NoteEditorPage = () => {
   ...
   const [activeCollaborators, setActiveCollaborators] = useState({}); 
    const otherRealtimeCollaborators = Object.keys(activeCollaborators).filter(collab => collab !== user.username);
        ...
        const fetchNoteAndUsers = async () => {
            ...
            if (id) {
                try {
                    ...
                    setCurrentNoteCollaborators(fetchedNote.collaborators ? fetchedNote.collaborators.map(c => c._id) : []);
                    setOriginalNoteOwner(fetchedNote.owner);
            ...
            try {
                ...
                const filteredUsers = allUsers.filter(u =>
                    u._id !== user._id &&
                    (fetchedNote ? u._id !== fetchedNote.owner._id : true) && 
                    (!fetchedNote || !fetchedNote.collaborators.some(collab => collab._id === u._id))                );
...
const userToRestore = availableUsers.find(u => u._id === userIdToRemove) ||
                              (originalNoteOwner && originalNoteOwner._id === userIdToRemove ? originalNoteOwner : null);        if (userToRestore) {
    ...
    if (error.includes('Not authorized to view this note') && !canEdit && !isNewNote) {
        return <div className="text-center mt-8 text-red-600 font-semibold">You are not authorized to view or edit this note.</div>;
    }
                            ...
                            {currentNoteCollaborators.length === 0 ? (
                                ...
                                <ul className="list-disc list-inside">
                                    {currentNoteCollaborators.map(collabId => {
                                        ...
                                        const collabUser = availableUsers.find(u => u._id === collabId) ||
